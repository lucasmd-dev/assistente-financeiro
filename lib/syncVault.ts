// Sincronização oculta criptografada via Firestore REST — PORTADO BYTE-A-BYTE de src/utils/syncVault.js.
// Segurança crítica: NÃO alterar iterações PBKDF2 (250000), envelope {v:1}, salt/iv, last-write-wins.
// Ativado pelo console: window.assistenteSync.ativar({ firebaseConfig, syncId, chave }).
...
interface SyncRuntime {
  ...
  setData: ((data: FinanceData) => void) | null;
  sessionId: number;
  needsBoot: boolean;
  pushTimer: number | null;
  pendingData: FinanceData | null;
  ignoreNextLocalChange: boolean;
}
...
const readJson = (key: string, fallback: any = null): any => {
  try {
...
const createRandomId = (length = 32): string => {
...
const normalizeFirebaseConfig = (input: any): any => {
...
const readMeta = (): any => readJson(META_KEY, {});

const saveMeta = (patch: any): any => {
  const next = {
    ...readMeta(),
    ...patch,
...
const deriveKey = async (chave: string, salt: Uint8Array): Promise<CryptoKey> => {
  assertCryptoSupport();
...
const encryptPayload = async (value: string, chave: string): Promise<string> => {
  const encoder = new TextEncoder();
...
const decryptPayload = async (payload: string, chave: string): Promise<string> => {
...
const firestoreUrl = (config: any, params: [string, string][] = []): string => {
...
const requestFirestore = async (url: string, options: RequestInit = {}): Promise<any> => {
  const response = await fetch(url, options);
  const text = await response.text();
  const body = text ? JSON.parse(text) : null;

  if (!response.ok) {
    const error: any = new Error(body?.error?.message || `Erro ${response.status} no Firestore.`);
...
const getFieldString = (field: any): string => {
...
const getRemoteVault = async (config: any): Promise<any> => {
  try {
...
const writeRemoteVault = async (config: any, payload: string): Promise<any> => {
...
const isEmptyRemotePayload = (payload: string): boolean => {
  const value = String(payload || '').trim();
  return !value || value === '{}';
};

const prepareDataForSync = (data: any): any => ({
  ...data,
  apiKey: '',
});

const buildSyncedData = (remoteData: any): any => ({
  ...remoteData,
  apiKey: runtime.getData?.()?.apiKey || '',
});

const pushData = async (data: any, options: any = {}): Promise<any> => {
...
const pullData = async (options: any = {}): Promise<any> => {
  const config = runtime.config || readConfig();
  ...
  const shouldApply =
    options.force || !localUpdatedAt || new Date(remoteSavedAt) > new Date(localUpdatedAt);
...
const startSync = async (): Promise<any> => {
  const config = runtime.config || readConfig();
...
const schedulePush = (data: any): void => {
...
export const notifySyncDataChange = (data: FinanceData): void => {
  const config = runtime.config || readConfig();
...
const createConsoleApi = (): any => ({
  ativar: async (input: any) => {
...
export const installSyncVault = ({ getData, setData }: InstallArgs): (() => void) => {
  ...
  (window as any).assistenteSync = api;
  ...
  return () => {
    ...
    if ((window as any).assistenteSync === api) {
      delete (window as any).assistenteSync;
    }
  };
};
